...
template<> struct hash<cashew::IString> : public __unary_function<cashew::IString, size_t> {
  size_t operator()(const cashew::IString& str) const {
    return std::hash<size_t>{}(size_t(str.str));
  }
};

template<> struct equal_to<cashew::IString> : public __binary_function<cashew::IString, cashew::IString, bool> {
